...
@Controller
@ServerEndpoint(value = "/chat/R/{username}")  // this is Websocket url
public class ChatControllerR {
    ...
    public void setUserRepository(UserRepository repo) {
        ChatControllerR.userRepository = repo;
    }
    ...
    public void onMessage(Session session, String message) throws IOException {
        ...
        if (user != null && user.getMute()) {
            // User is muted
            sendMessageToPArticularUser(username, "You are muted in R category and cannot send messages.");
            return;
        }
        ...
        msgRepo.save(new Message(username, message, "R"));
    }
    ...
    private String getChatHistory() {
        List<Message> messages = msgRepo.findByType("R");
